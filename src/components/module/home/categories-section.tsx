...
//   return (
//     <section className="relative py-5 md:py-10 bg-[#f0fdf8] dark:bg-[#020810] overflow-hidden px-10">
//       {/* ───────── BACKGROUND (MATCH HERO) ───────── */}

//       {/* Grid / dot pattern */}
//       <div
//         className="absolute inset-0 opacity-[0.55] dark:opacity-[0.56]"
//         style={{
//           backgroundImage: `
...
//       <div
//         className="absolute inset-0 opacity-[1.56] dark:opacity-[0.55] .bg-grid-light .dark .bg-grid-light"
//         style={{
//           backgroundImage: `
...
//       />
//       <div
//         className="absolute bottom-[-10%] right-[-5%] w-[400px] h-[400px] rounded-full blur-[80px]"
//         style={{
//           background:
...
//                 <p
//                   className="
//                   relative z-10 mt-3 text-sm font-semibold text-center
...
import Link from "next/link";
import {
  Pill, Heart, Brain, Eye, Bone, Baby, Leaf,
  Droplets, Thermometer, Shield, Stethoscope,
  Wind, FlaskConical, Sun, Zap, Tag,
} from "lucide-react";

const PALETTES = [
  { icon: "bg-emerald-500/10 dark:bg-emerald-400/10 text-emerald-600 dark:text-emerald-400", border: "hover:border-emerald-400/40 dark:hover:border-emerald-600/40", glow: "rgba(16,185,129,0.15)", dot: "bg-emerald-500" },
  { icon: "bg-blue-500/10 dark:bg-blue-400/10 text-blue-600 dark:text-blue-400", border: "hover:border-blue-400/40 dark:hover:border-blue-600/40", glow: "rgba(59,130,246,0.15)", dot: "bg-blue-500" },
  { icon: "bg-violet-500/10 dark:bg-violet-400/10 text-violet-600 dark:text-violet-400", border: "hover:border-violet-400/40 dark:hover:border-violet-600/40", glow: "rgba(139,92,246,0.15)", dot: "bg-violet-500" },
  { icon: "bg-rose-500/10 dark:bg-rose-400/10 text-rose-600 dark:text-rose-400", border: "hover:border-rose-400/40 dark:hover:border-rose-600/40", glow: "rgba(244,63,94,0.13)", dot: "bg-rose-500" },
  { icon: "bg-amber-500/10 dark:bg-amber-400/10 text-amber-600 dark:text-amber-400", border: "hover:border-amber-400/40 dark:hover:border-amber-600/40", glow: "rgba(245,158,11,0.15)", dot: "bg-amber-500" },
  { icon: "bg-teal-500/10 dark:bg-teal-400/10 text-teal-600 dark:text-teal-400", border: "hover:border-teal-400/40 dark:hover:border-teal-600/40", glow: "rgba(20,184,166,0.15)", dot: "bg-teal-500" },
  { icon: "bg-orange-500/10 dark:bg-orange-400/10 text-orange-600 dark:text-orange-400", border: "hover:border-orange-400/40 dark:hover:border-orange-600/40", glow: "rgba(249,115,22,0.13)", dot: "bg-orange-500" },
  { icon: "bg-cyan-500/10 dark:bg-cyan-400/10 text-cyan-600 dark:text-cyan-400", border: "hover:border-cyan-400/40 dark:hover:border-cyan-600/40", glow: "rgba(6,182,212,0.15)", dot: "bg-cyan-500" },
] as const;
...
const ICONS = [
  ...
  { keywords: ["diabetes", "insulin", "blood sugar", "glucose", "antidiabetic"], Icon: Droplets },
  { keywords: ["allergy", "antihistamine", "sinus"], Icon: Wind },
  { keywords: ["gastro", "stomach", "digestive", "acid", "ulcer", "antacid"], Icon: FlaskConical },
  { keywords: ["heart", "cardiac", "blood pressure", "cholesterol"], Icon: Heart },
  ...
  { keywords: ["respiratory", "lung", "asthma", "cough", "breath"], Icon: Wind },
...
function pickIcon(name: string) {
  ...
  return ICONS.find(({ keywords }) => keywords.some((k) => lower.includes(k)))?.Icon ?? Tag;
}
...
export async function CategoriesSection() {
  ...
  return (
    <section className="relative py-10 md:py-14 bg-[#f0fdf8] dark:bg-[#020810] overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        />
        <div className="absolute rounded-full" style={{ top: "-10%", left: "-5%", width: 380, height: 380, background: "radial-gradient(circle,rgba(16,185,129,0.12),transparent 70%)", filter: "blur(70px)" }} />
        <div className="absolute rounded-full" style={{ bottom: "-10%", right: "-5%", width: 380, height: 380, background: "radial-gradient(circle,rgba(20,184,166,0.09),transparent 70%)", filter: "blur(70px)" }} />
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-9">
          ...
          <p className="text-slate-500 dark:text-slate-500 mt-2 text-xs max-w-xs mx-auto leading-relaxed">
            Find exactly what you need from our curated pharmaceutical categories.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {categories.map((category, idx) => {
            ...
            return (
              <Link key={category.id} href={`/shop?categoryId=${category.id}`} className="group">
                <div
                  className={`
                ...
                >
                  ...
                  <div
                    ...
                    style={{ background: `radial-gradient(circle at 50% 70%,${palette.glow},transparent 70%)` }}
                  />
                  ...
                  <div className="relative z-10 opacity-0 -translate-y-0.5 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200">
                    <svg className="w-3 h-3 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 12 12" stroke="currentColor" strokeWidth={2.5}>
                      <path d="M1 6h10M6 1l5 5-5 5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </div>

                  {/* Bottom accent */}
                  <div className={`absolute bottom-0 inset-x-0 h-[2px] ${palette.dot} opacity-0 group-hover:opacity-80 transition-opacity duration-300 rounded-b-xl`} />
                </div>
              </Link>
            );
          })}
  ...
  <Link href="/shop" className="group inline-block">
    <div
      ...
      style={{
        background: "linear-gradient(135deg, rgba(16,185,129,0.5), rgba(20,184,166,0.3), rgba(16,185,129,0.5))",
      }}
    >
      <div
        className="
